...
export async function listNotificationOperations(filters: { status?: "pending" | "queued" | "sending" | "delivered" | "failed" | "retrying" | "suppressed" | "cancelled" | "expired" | "unavailable"; channel?: "in_app" | "email" | "sms" | "push" } = {}) {
  const db = await getDb();
  if (!db) return [];
  const where = [filters.status ? eq(notificationDeliveries.status, filters.status) : undefined, filters.channel ? eq(notificationDeliveries.channel, filters.channel) : undefined].filter(Boolean) as any[];
  return db.select({ id: notificationDeliveries.id, channel: notificationDeliveries.channel, provider: notificationDeliveries.provider, status: notificationDeliveries.status, retryCount: notificationDeliveries.retryCount, nextRetryAt: notificationDeliveries.nextRetryAt, createdAt: notificationDeliveries.createdAt, eventType: notificationEvents.eventType, priority: notificationEvents.priority, notificationClass: notificationEvents.notificationClass }).from(notificationDeliveries).innerJoin(notificationEvents, eq(notificationDeliveries.notificationEventId, notificationEvents.id)).where(where.length ? and(...where) : undefined).orderBy(desc(notificationDeliveries.createdAt)).limit(100);
}
