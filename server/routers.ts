...
import { FAMILY_PERMISSIONS, acceptFamilyInvitation, createFamilyInvitation, declineFamilyInvitation, getFamilyParticipantDashboard, listFamilyMetadataForOperations, listMemberFamilyCircle, reissueFamilyInvitation, removeFamilyParticipant, reportFamilyParticipant, requestFamilyAcknowledgment, respondToFamilyAcknowledgment, restrictFamilyParticipant, reviewWaliGuardianVerification, setFamilyPermission, sharePotentialMatch, submitFamilyFeedback, withdrawFamilyShare } from "./familyService";
...
import { cancelSubscriptionRenewal, getMemberBilling, hasEntitlement, initiatePayment, listBillingConfiguration, listFinanceTransactions, listMembershipOptions, listReconciliationQueue, listRefundRequests, requestMemberRefund, requestRefund, reviewRefundRequest, saveBillingPlanConfiguration, saveMemberBillingSettings, savePaymentProviderAvailability } from "./billingService";
...
export const appRouter = router({
  ...
  messaging: router({
    ...
    preferences: protectedProcedure.input(z.object({ conversationId: z.number().int().positive(), isMuted: z.boolean().optional(), readReceiptsEnabled: z.boolean().optional() })).mutation(async ({ ctx, input }) => setConversationPreference((await requireProfile(ctx.user.id)).id, input.conversationId, input)),
	    setState: protectedProcedure.input(z.object({ conversationId: z.number().int().positive(), state: z.enum(["active", "paused", "closed"]) })).mutation(async ({ ctx, input }) => setConversationState((await requireProfile(ctx.user.id)).id, input.conversationId, input.state)),
  ...
  safety: router({
    ...
    setConversationModeration: protectedProcedure.input(z.object({ conversationId: z.number().int().positive(), state: z.enum(["restricted", "active"]), reason: z.string().max(500).optional() })).mutation(async ({ ctx, input }) => {
      ...
      await requireOperationalAccess(ctx.user, ["trust_safety", "platform_admin"]);
	    ...
	    notificationOperations: protectedProcedure.query(async ({ ctx }) => { await requireOperationalAccess(ctx.user, ["support_agent", "trust_safety", "platform_admin"]); return listNotificationOperations(); }),
	    ...
	    supportTickets: staffOnlyProcedure.input(z.object({ status: z.enum(["new", "open", "waiting_for_member", "waiting_for_staff", "escalated", "resolved", "closed"]).optional() })).query(async ({ ctx, input }) => listSupportTickets(ctx.user.id, input.status)),
	    ...
	    updateSupportTicket: staffOnlyProcedure.input(z.object({ ticketId: z.number().int().positive(), status: z.enum(["new", "open", "waiting_for_member", "waiting_for_staff", "escalated", "resolved", "closed"]).optional(), assignedStaffProfileId: z.number().int().positive().nullable().optional(), resolution: z.string().trim().max(1000).optional() })).mutation(async ({ ctx, input }) => { const { ticketId, ...change } = input; return updateSupportTicket(ctx.user.id, ticketId, change); }),
