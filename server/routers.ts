...
import { getCompatibilityExplanation, getCompatibilityPreferences, getCuratedDiscovery, getDiscoveryFilters, listProfileFieldVisibilities, saveCompatibilityPreferences, saveDiscoveryFilters, saveProfileFieldVisibilities } from "./compatibilityService";
...
import { acceptStaffInvitation, createMemberSupportTicket, createOperationalApproval, createOperationalIncident, createStaffSessionControl, createSupportTicket, decideOperationalApproval, getEffectiveStaffAccess, getOperationalMemberSummary, inviteStaff, listAssignableSupportStaff, listCurrentStaffPermissions, listFeatureFlags, listMemberSupportTickets, listOperationalApprovals, listOperationalAudit, listOperationalIncidents, listOperationsOverview, listStaffDirectory, listSupportTickets, proposeStaffChange, reopenMemberSupportTicket, requireOperationalPermission, withdrawMemberSupportTicket, revokeStaffSession, searchOperationalMembers, updateOperationalIncident, updateSupportTicket } from "./adminOperationsService";
...
import { cancelMemberDataRightsRequest, getMemberAccountSummary, getMemberSecuritySessions, pauseMemberAccount, reactivateMemberAccount, requestMemberDataRights, revokeMemberSecuritySession, revokeOtherMemberSecuritySessions } from "./accountService";
...
export const appRouter = router({
  ...
  account: router({
    summary: betaMemberProcedure.query(({ ctx }) => getMemberAccountSummary(ctx.user.id, ctx.user.sessionReferenceHash)),
    sessions: betaMemberProcedure.query(({ ctx }) => getMemberSecuritySessions(ctx.user.id, ctx.user.sessionReferenceHash)),
    revokeSession: betaMemberProcedure.input(z.object({ sessionId: z.number().int().positive(), expectedUpdatedAt: z.date() })).mutation(({ ctx, input }) => revokeMemberSecuritySession(ctx.user.id, ctx.user.sessionReferenceHash, input)),
    revokeOtherSessions: betaMemberProcedure.input(z.object({ sessions: z.array(z.object({ sessionId: z.number().int().positive(), expectedUpdatedAt: z.date() })).min(1).max(20) })).mutation(({ ctx, input }) => revokeOtherMemberSecuritySessions(ctx.user.id, ctx.user.sessionReferenceHash, input.sessions)),
    ...
    requestDataRights: betaMemberProcedure.input(z.object({ requestType: z.enum(["data_export", "account_deletion"]), expectedUpdatedAt: z.date().optional() })).mutation(({ ctx, input }) => requestMemberDataRights(ctx.user.id, input.requestType, input.expectedUpdatedAt, ctx.user.sessionReferenceHash)),
    cancelDataRightsRequest: betaMemberProcedure.input(z.object({ requestId: z.number().int().positive(), expectedUpdatedAt: z.date().optional() })).mutation(({ ctx, input }) => cancelMemberDataRightsRequest(ctx.user.id, input.requestId, input.expectedUpdatedAt, ctx.user.sessionReferenceHash)),
  }),
  support: router({
    mine: betaMemberProcedure.query(({ ctx }) => listMemberSupportTickets(ctx.user.id)),
    create: betaMemberProcedure.input(z.object({ category: z.enum(["account_access", "profile", "verification", "membership", "payment", "notifications", "family_circle", "technical_issue", "safety_concern", "other"]), subject: z.string().trim().min(3).max(200), description: z.string().trim().min(10).max(5000), idempotencyKey: z.string().trim().min(16).max(100) })).mutation(({ ctx, input }) => createMemberSupportTicket(ctx.user.id, input)),
    withdraw: betaMemberProcedure.input(z.object({ ticketId: z.number().int().positive(), expectedUpdatedAt: z.coerce.date() })).mutation(({ ctx, input }) => withdrawMemberSupportTicket(ctx.user.id, input.ticketId, input.expectedUpdatedAt)),
    reopen: betaMemberProcedure.input(z.object({ ticketId: z.number().int().positive(), expectedUpdatedAt: z.coerce.date() })).mutation(({ ctx, input }) => reopenMemberSupportTicket(ctx.user.id, input.ticketId, input.expectedUpdatedAt)),
  }),
  profile: router({
  ...
  discovery: router({
      ...
      return getDiscoveryProfiles(profile.id, input);
	    ...
	    curated: protectedProcedure.input(z.object({ cursor: z.number().int().positive().optional(), limit: z.number().int().min(1).max(24).optional(), collection: z.enum(["recommended", "new", "recently_updated", "verified", "potentially_compatible"]).optional(), query: z.string().trim().max(80).optional(), minAge: z.number().int().min(18).max(60).optional(), maxAge: z.number().int().min(18).max(60).optional(), gender: z.enum(["woman", "man", "self_described"]).optional(), religion: z.enum(["muslim", "christian"]).optional(), country: z.string().trim().max(100).optional(), residenceType: z.enum(["gambia", "diaspora"]).optional(), city: z.string().max(100).optional(), maritalStatus: z.enum(["never_married", "married", "divorced", "widowed"]).optional(), hasChildren: z.boolean().optional(), relocationWillingness: z.enum(["open", "within_gambia", "not_open", "discuss"]).optional(), polygynyOpenness: z.enum(["open", "not_open", "discuss", "not_applicable"]).optional(), verifiedOnly: z.boolean().optional() }).optional()).query(async ({ ctx, input }) => getCuratedDiscovery((await requireProfile(ctx.user.id)).id, input)),
    filters: protectedProcedure.query(async ({ ctx }) => getDiscoveryFilters((await requireProfile(ctx.user.id)).id)),
	    saveFilters: protectedProcedure.input(z.object({ query: z.string().trim().max(80).nullable().optional(), minAge: z.number().int().min(18).max(60).nullable().optional(), maxAge: z.number().int().min(18).max(60).nullable().optional(), gender: z.enum(["woman", "man", "self_described"]).nullable().optional(), religion: z.enum(["muslim", "christian"]).nullable().optional(), country: z.string().trim().max(100).nullable().optional(), residenceType: z.enum(["gambia", "diaspora"]).nullable().optional(), maritalStatus: z.enum(["never_married", "married", "divorced", "widowed"]).nullable().optional(), hasChildren: z.boolean().nullable().optional(), relocationWillingness: z.enum(["open", "within_gambia", "not_open", "discuss"]).nullable().optional(), polygynyOpenness: z.enum(["open", "not_open", "discuss", "not_applicable"]).nullable().optional(), verifiedOnly: z.boolean().optional(), expectedUpdatedAt: z.coerce.date().optional() })).mutation(async ({ ctx, input }) => { const profile = await requireProfile(ctx.user.id); const { expectedUpdatedAt, ...filters } = input; return saveDiscoveryFilters(profile.id, filters, expectedUpdatedAt); }),
  }),
  compatibility: router({
    ...
    explain: protectedProcedure.input(z.object({ profileId: z.number().int().positive() })).query(async ({ ctx, input }) => getCompatibilityExplanation((await requireProfile(ctx.user.id)).id, input.profileId)),
	  ...
	  interests: router({
	    ...
	    send: protectedProcedure.input(z.object({ recipientProfileId: z.number().int().positive(), message: z.string().max(500).optional() })).mutation(async ({ ctx, input }) => { const profile = await requireProfile(ctx.user.id); const interest = await createInterest(profile.id, input.recipientProfileId, input.message); await withdrawRecommendationsForProfilePair(profile.id, input.recipientProfileId, "interest_started"); return interest; }),
	    respond: protectedProcedure.input(z.object({ interestId: z.number().int().positive(), response: z.enum(["accepted", "declined"]), expectedUpdatedAt: z.coerce.date().optional() })).mutation(async ({ ctx, input }) => { const profile = await requireProfile(ctx.user.id); const result = await respondToInterest(profile.id, input.interestId, input.response, input.expectedUpdatedAt); if (result.matched) await withdrawRecommendationsForProfilePair(profile.id, result.otherProfileId, "connection_created"); return result; }),
	    withdraw: protectedProcedure.input(z.object({ interestId: z.number().int().positive(), expectedUpdatedAt: z.coerce.date().optional() })).mutation(async ({ ctx, input }) => withdrawInterest((await requireProfile(ctx.user.id)).id, input.interestId, input.expectedUpdatedAt)),
  ...
  billing: router({
	    ...
	    initiate: protectedProcedure.input(z.object({ membershipPriceId: z.number().int().positive(), provider: z.string().trim().min(2).max(80), idempotencyKey: z.string().trim().min(16).max(96), acknowledgedTerms: z.literal(true), returnUrl: z.string().url().max(500).optional() })).mutation(async ({ ctx, input }) => { const profile = await requireProfile(ctx.user.id); if (profile.profileStatus === "paused" || profile.profileStatus === "suspended") throw new Error("Billing checkout is unavailable while this account has limited access. No transaction or membership change was created."); return initiatePayment(profile.id, ctx.user.id, input); }),
	    cancelRenewal: protectedProcedure.input(z.object({ subscriptionId: z.number().int().positive(), expectedUpdatedAt: z.date().optional() })).mutation(async ({ ctx, input }) => cancelSubscriptionRenewal((await requireProfile(ctx.user.id)).id, ctx.user.id, input.subscriptionId, input.expectedUpdatedAt)),
  ...
  safety: router({
    ...
    unblock: betaMemberProcedure.input(z.object({ blockedProfileId: z.number().int().positive(), expectedCreatedAt: z.date().optional() })).mutation(async ({ ctx, input }) => { const profile = await requireProfile(ctx.user.id); const result = await unblockProfile(profile.id, input.blockedProfileId, input.expectedCreatedAt); return result; }),
    ...
    updateReport: betaMemberProcedure.input(z.object({ reportId: z.number().int().positive(), details: z.string().trim().min(1).max(2000), expectedUpdatedAt: z.date().optional() })).mutation(async ({ ctx, input }) => updateMemberSafetyReport(ctx.user.id, input.reportId, input.details, input.expectedUpdatedAt)),
    withdrawReport: betaMemberProcedure.input(z.object({ reportId: z.number().int().positive(), expectedUpdatedAt: z.date().optional() })).mutation(async ({ ctx, input }) => withdrawMemberSafetyReport(ctx.user.id, input.reportId, input.expectedUpdatedAt)),
    ...
    withdrawAppeal: betaMemberProcedure.input(z.object({ appealId: z.number().int().positive(), expectedUpdatedAt: z.date().optional() })).mutation(async ({ ctx, input }) => withdrawSafetyAppeal(ctx.user.id, input.appealId, input.expectedUpdatedAt)),
  }),
    ...
    setConversationModeration: protectedProcedure.input(z.object({ conversationId: z.number().int().positive(), state: z.enum(["restricted", "active"]), reason: z.string().max(500).optional() })).mutation(async ({ ctx, input }) => {
      ...
      await requireOperationalAccess(ctx.user, ["trust_safety", "platform_admin"]);
	    ...
	    safetyCaseDetail: protectedProcedure.input(z.object({ reportId: z.number().int().positive() })).query(async ({ ctx, input }) => { await requireOperationalAccess(ctx.user, ["trust_safety", "platform_admin"]); return getSafetyCaseDetail(ctx.user.id, input.reportId); }),
	    ...
	    approveSafetyEnforcement: protectedProcedure.input(z.object({ enforcementActionId: z.number().int().positive(), expectedUpdatedAt: z.coerce.date().optional() })).mutation(async ({ ctx, input }) => { await requireOperationalAccess(ctx.user, ["trust_safety", "platform_admin"]); return approveSafetyEnforcement(ctx.user.id, input.enforcementActionId, input.expectedUpdatedAt); }),
	    revokeSafetyEnforcement: protectedProcedure.input(z.object({ enforcementActionId: z.number().int().positive(), reason: z.string().trim().min(3).max(500), expectedUpdatedAt: z.coerce.date().optional() })).mutation(async ({ ctx, input }) => { await requireOperationalAccess(ctx.user, ["trust_safety", "platform_admin"]); return revokeSafetyEnforcement(ctx.user.id, input.enforcementActionId, input.reason, input.expectedUpdatedAt); }),
    ...
    supportTickets: staffOnlyProcedure.input(z.object({ status: z.enum(["new", "open", "waiting_for_member", "waiting_for_staff", "escalated", "resolved", "closed", "withdrawn"]).optional(), category: z.enum(["account_access", "profile", "verification", "membership", "payment", "notifications", "family_circle", "technical_issue", "safety_concern", "other"]).optional(), assignment: z.enum(["assigned", "unassigned"]).optional() })).query(async ({ ctx, input }) => listSupportTickets(ctx.user.id, input)),
    ...
    updateSupportTicket: staffOnlyProcedure.input(z.object({ ticketId: z.number().int().positive(), status: z.enum(["new", "open", "waiting_for_member", "waiting_for_staff", "escalated", "resolved", "closed", "withdrawn"]).optional(), assignedStaffProfileId: z.number().int().positive().nullable().optional(), resolution: z.string().trim().max(1000).optional(), expectedUpdatedAt: z.coerce.date() })).mutation(async ({ ctx, input }) => { const { ticketId, ...change } = input; return updateSupportTicket(ctx.user.id, ticketId, change); }),
