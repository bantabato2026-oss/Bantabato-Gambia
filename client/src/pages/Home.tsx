...
import { Reveal, Stagger } from "@/components/ExperienceMotion";
    ...
    <main>
      <section className="hero-section">
        ...
        <div className="container relative grid items-end gap-12 pb-16 pt-16 lg:grid-cols-[1.15fr_.85fr] lg:pb-24 lg:pt-24">
          <Reveal><div className="max-w-3xl"><div className="eyebrow"><span className="eyebrow-dot" /> Matrimony, with meaning</div><h1 className="hero-title">Where love<br />takes <em>root.</em></h1><p className="hero-copy">A trusted place for Gambians at home and abroad to meet with faith, clarity, and a sincere intention toward marriage.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button asChild size="lg" className="btn-gold h-12 px-6"><Link href="/register">Begin with intention <ArrowRight size={17} /></Link></Button><Button asChild size="lg" variant="outline" className="h-12 border-forest/20 bg-transparent px-6 text-forest hover:bg-forest/5"><Link href="/how-it-works">How Bantabato works</Link></Button></div><p className="mt-6 flex items-center gap-2 text-sm text-forest/70"><LockKeyhole size={15} /> Private by default. Respectful by design.</p></div></Reveal>
          <Reveal delay="motion-delay-2"><div className="hero-card"><div className="hero-card-top"><p className="font-display text-2xl text-cream">A considered beginning</p><div className="hero-orbit"><div /><div /><div /></div></div><div className="mt-auto"><div className="divider-gold" /><p className="mt-5 text-sm leading-6 text-cream/70">A shared path begins with mutual respect, verified identity, and the freedom to move at your own pace.</p><div className="mt-5 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-gold"><CheckCircle2 size={15} /> Designed for serious adults</div></div></div></Reveal>
        </div>
      </section>
      <section className="section-soft"><div className="container"><Reveal><div className="max-w-2xl"><p className="eyebrow text-gold-dark">Built around what matters</p><h2 className="section-title mt-4">A quieter, more dignified way to meet.</h2><p className="mt-5 max-w-xl leading-7 text-muted-foreground">Bantabato is made for adults seeking a meaningful marriage. Every detail—from privacy settings to family involvement—is shaped to support a respectful journey.</p></div></Reveal><Stagger className="mt-12 grid gap-5 md:grid-cols-3">{pillars.map(pillar => <div key={pillar.title} className="value-card"><div className="icon-plate"><pillar.icon size={22} /></div><h3 className="mt-7 font-display text-2xl text-ink">{pillar.title}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{pillar.text}</p></div>)}</Stagger></div></section>
