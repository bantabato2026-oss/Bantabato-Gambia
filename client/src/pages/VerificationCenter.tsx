...
export default function VerificationCenter() {
  ...
  const upload = trpc.uploads.uploadIdentityDocument.useMutation({
    ...
    onError: error => {
      setFileReadError(/changed before this submission|changed before/i.test(error.message) ? "Your verification status changed before this submission. No document was sent. Refresh the page, review the current status, then try again if a new submission is available." : "Your document was not submitted. No verification result has changed; review your connection and retry when ready.");
    },
  });
  ...
  const submit = async () => {
    ...
    try {
      ...
      upload.mutate({ documentType, dataUrl, expectedLatestVerificationId: latestIdentityRecord?.id ?? null });
    } catch {
  ...
  return <MemberShell eyebrow="Verification" title="Trust, reviewed with care." description="A verified indicator is available only after a qualified administrator completes a private identity-document review. It confirms that review only; it does not guarantee another member’s conduct, compatibility, or outcome.">
    ...
    <PlainLanguagePanel className="mt-7" title="Verification in three clear steps" what={pending ? "Your document is waiting for review." : identityApproved ? "Your current document review is approved." : "Choose a National ID or Passport when you are ready."} why="A private document review can support an Identity reviewed indicator. It does not promise safety, character, or compatibility." next={pending ? "Wait for a private update. Do not submit another document while this review is open." : identityApproved ? "No action is needed unless Bantabato asks you to update your document." : "Select one supported document, check the details, then submit it for manual review."} />
    ...
    <div className="mt-7"><VoiceGuidancePanel title="Verification guidance" textAlternative="Use the three steps above. Select one supported document only when you are ready. After you submit it, wait for a private status update. You can continue without voice guidance." /></div>
