...
export function AdminShell({ eyebrow = "Bantabato operations", title, description, children }: { eyebrow?: string; title: string; description: string; children: React.ReactNode }) {
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><Brand inverse /><p className="mt-10 text-xs font-semibold uppercase tracking-[.16em] text-gold">Administration</p><nav className="mt-4 space-y-1" aria-label="Staff workspace navigation">{visibleNavigation.map(item => { const Icon = item.icon; const active = item.href === location || (item.href !== "/admin" && location.startsWith(item.href)); return <Link key={item.label} href={item.href} aria-current={active ? "page" : undefined} className={`admin-nav-item ${active ? "admin-nav-active" : ""}`}><Icon size={17} /> {item.label}</Link>; })}</nav><Link href="/app" className="mt-auto flex items-center gap-2 text-sm text-cream/70 hover:text-cream"><ArrowLeft size={16} /> Member space</Link></aside><main className="admin-main"><nav className="admin-mobile-nav" aria-label="Staff workspace navigation"><span className="admin-mobile-nav-label">Workspace</span><div>{visibleNavigation.map(item => { const Icon = item.icon; const active = item.href === location || (item.href !== "/admin" && location.startsWith(item.href)); return <Link key={item.label} href={item.href} aria-current={active ? "page" : undefined} className={`admin-mobile-nav-item ${active ? "admin-mobile-nav-active" : ""}`}><Icon size={15} /> {item.label}</Link>; })}</div></nav><p className="eyebrow text-gold-dark">{eyebrow}</p><h1 className="mt-3 font-display text-4xl text-ink">{title}</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">{description}</p>{workspaceState}</main></div>;
}
