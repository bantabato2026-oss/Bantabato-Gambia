...
import { Bell, Camera, ChevronRight, CircleUserRound, Compass, CreditCard, Globe2, HeartHandshake, Home, Lightbulb, LogOut, Menu, MessageCircle, Settings, ShieldCheck, SlidersHorizontal, UsersRound } from "lucide-react";
    ...
    <div className="min-h-screen bg-app">
      <aside className={`member-sidebar ${menuOpen ? "member-sidebar-open" : ""}`}>
        ...
        <div className="mt-auto border-t border-forest/10 p-4">
          <button onClick={logout} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium text-muted-foreground transition-colors hover:bg-forest/5 hover:text-forest"><LogOut size={17} /> Sign out</button>
        </div>
      </aside>
...
function NavItem({ item, active, navigate }: { item: { label: string; href: string; icon: React.ComponentType<{ size?: number }> }; active: boolean; navigate: (to: string) => void }) {
  ...
  return <button onClick={() => navigate(item.href)} className={`member-nav-item ${active ? "member-nav-active" : ""}`}><Icon size={18} /><span>{item.label}</span></button>;
}
