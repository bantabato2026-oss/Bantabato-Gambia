...
export function ProfileReadinessPanel({ eligibility, verificationStatus, hasPreferences = false, className = "", compact = false }: Props) {
  ...
  const rows = [
    ...
    { icon: eligibility?.discoveryEligible ? CheckCircle2 : CircleAlert, label: "Discovery", state: discoveryState, detail: eligibility?.discoveryEligible ? "Your current profile can take part in discovery, subject to privacy, safety, and relationship boundaries." : eligibility?.title || "Discovery remains unavailable until the current server-authoritative requirements are met.", tone: eligibility?.discoveryEligible ? "complete" as const : "action" as const },
	  ...
	  const journeyState = eligibility?.journeyState || eligibility?.onboardingState;
	  const activationAction = getActivationJourneyAction(eligibility);
		  return <section className={`surface-card ${className}`} aria-labelledby="profile-readiness-title"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow text-gold-dark">Factual readiness</p><h2 id="profile-readiness-title" className="mt-2 font-display text-2xl text-ink">Clear next steps, not a score.</h2></div>{verification.tone === "pending" ? <Clock3 className="text-gold-dark" size={21} aria-hidden="true" /> : null}</div><p className="mt-3 text-sm leading-6 text-muted-foreground">This view shows only the steps that are currently complete or still needed. It does not rank you, show private safety information, or promise an outcome.</p>{journeyState ? <p role="status" className="mt-3 rounded-xl bg-cream/70 px-3 py-2 text-xs font-semibold text-forest">Current journey state: {journeyState.replace(/_/g, " ").toLowerCase()}.</p> : null}{!compact ? <PlainLanguagePanel className="mt-5" title="Your next profile step" what={eligibility?.profileComplete ? "Your main profile details are complete." : eligibility?.title || "Some profile details or photos are still needed."} why="These details help keep introductions private, clear, and respectful." next={eligibility?.discoveryEligible ? "Discovery is currently available. You can review members at your own pace." : activationAction.unavailable ? "This account state is unavailable for discovery. Review the available account status without private enforcement details." : activationAction.label} /> : null}<div className="mt-5 space-y-3">{(compact ? rows.slice(1) : rows).map(row => <ReadinessRow key={row.label} {...row} />)}</div>{!compact ? <><VoiceGuidancePanel title="Readiness guidance" textAlternative="Read each step in this readiness list. Choose one unfinished item, complete it, and return here. Voice guidance is not connected, so the written steps remain the guide." /><div className="mt-5 flex flex-wrap gap-2"><Button asChild size="sm" variant="outline"><Link href="/app/photos">Manage photos</Link></Button><Button asChild size="sm" variant="outline"><Link href="/app/verification">Review verification</Link></Button><Button asChild size="sm" variant="outline"><Link href="/app/compatibility">Review preferences</Link></Button>{!eligibility?.discoveryEligible ? <Button asChild size="sm" className="btn-forest"><Link href={activationAction.href}>{activationAction.label}</Link></Button> : <Button asChild size="sm" className="btn-forest"><Link href="/app/discover">Explore members</Link></Button>}</div></> : null}</section>;
}
